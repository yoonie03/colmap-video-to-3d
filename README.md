...
- 왜곡 보정 이미지(`workspace/<프로젝트>/dense/images`)로 균열을 검출하여 3D 카메라와 좌표계를 맞춥니다.
- 같은 보정 이미지 경로의 검출 요약이 있고 검출 설정이 같으며, 전체 마스크가 읽히고 보정 이미지와 크기가 같으면 AI 검출을 건너뜁니다. 이전 원본 이미지로 만든 마스크는 다시 생성합니다.
...
균열 후보 표시는 PLY 옆의 `point_cloud_cracks.crack_labels.npy` 파일을 사용합니다. 결과를 옮길 때 함께 복사하세요. 표식이 없는 이전 결과는 색상으로 균열을 추정하지 않으며, 이어하기로 다시 생성할 수 있습니다.
...
RTX 5070 Ti, 최대 1280px 조건에서 측정한 결과입니다. 기본값은 2fps이며 `fan_1`만 등록률 개선을 위해 5fps 결과를 사용했습니다.
...
| `bench_2` | 41.4초 | 82/83 | 693,035 | 1,537,722 | 2,962,465 | 모델 선별 수정 포함 25분 32초 |
| `fan_1` (5fps) | 20.4초 | 35/102 | 227,345 | 183,326 | 337,951 | End-to-end 10분 30초 |
